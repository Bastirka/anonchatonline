...
Repozitorijā ir arī `api/socket-io.mjs` un `vercel.json`. Tie eksportē HTTP/Socket.IO serveri kā Vercel Function; Vercel vidē klients pieslēdzas tieši funkcijas `/api/socket-io` ceļam, bet lokāli un Render vidē izmanto `/socket.io`. Sākumlapa un citi `public/` faili tiek pasniegti no Vercel CDN.
